...
const initialNodes = [
	{
		...
		data: {
			...
			function: () => {
				...
				console.log(
					"Webhook Trigger activated, sending data:",
					appointmentData,
				);
				return appointmentData;
			},
...
export default function FlowCanvas() {
	...
	const onConnect = useCallback(
		(params: any) =>
			setEdges((eds) =>
				addEdge({ ...params, data: { label: "New Connection" } }, eds),
			),
		[setEdges],
	);
	...
	const simulateFlow = useCallback(() => {
		...
		const appointmentSetterData =
			appointmentSetterNode.data as AppointmentSetterNodeData;
		...
		console.log(
			"Sending data through edge: Webhook Trigger → Appointment Setter",
		);
