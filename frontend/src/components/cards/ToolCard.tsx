...
export default function ToolCard({ tool }: { tool: any }) {
	...
	const { toggleTool, testTool, expanded, setExpanded, TOOL_ICONS } =
		useToolContext();

	// Get the appropriate icon for a tool, or fall back to the default Wrench icon
	const getToolIcon = (toolId: string) => {
		const IconComponent =
			TOOL_ICONS[toolId.toLowerCase()] || TOOL_ICONS.default;
		...
		>
			...
			<div className="flex items-start gap-3">
				...
				<div className="flex-1 pr-8">
					{" "}
					{/* Increased right padding to avoid overlap with expanded test button */}
					<h3 className="text-sm font-medium leading-none mb-1.5">{tool.id}</h3>
					...
					<div className="flex items-center mt-1 space-x-1">
						{tool.tags &&
							tool.tags.map((tag: string) => (
								<span
									key={tag}
									className="text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 px-1.5 py-0.5 rounded-full"
								>
									{tag}
								</span>
							))}
