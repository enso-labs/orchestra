...
export function AgentThreadHistoryDrawer({
	isOpen,
	onClose,
}: AgentThreadHistoryDrawerProps) {
	const { history, setMessages, setPayload, deleteThread, payload } =
		useChatContext();
						...
						{history?.threads
							...
							.map((thread: any) => (
								<div key={thread.thread_id} className="group relative">
									<button
										onClick={() =>
											handleThreadClick(thread.thread_id, thread.messages)
										}
									...
									>
										<div className="w-full pr-8">
											<p className="text-sm font-medium line-clamp-2 max-w-60">
												...
												typeof thread.messages[1]?.content === "string"
													? truncateFrom(
															thread.messages[1].content,
															"end",
															"...",
