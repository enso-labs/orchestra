		...
		>
			<div className="flex items-center justify-between mb-1">
				...
				<div onClick={onClick} className="cursor-pointer transition-all">
					{isSelected ? (
						<CheckCircleIcon
							className="text-blue-600 transition-opacity duration-200"
							size={16}
						/>
					) : (
						<PlusIcon
...
function TabContentMCP() {
	...
	const handleConfigSelect = (selectedConfig: any) => {
		try {
			...
			if (
				typeof selectedConfig.slug === "string" &&
				Object.prototype.hasOwnProperty.call(currentConfig, selectedConfig.slug)
			) {
				// If already exists, remove it
				const { [selectedConfig.slug as string]: removed, ...rest } =
					...
					<ScrollArea className="h-[calc(100vh-300px)] pr-3 border rounded-md bg-muted/30 p-4">
						<div className="space-y-3">
							{mcpInfo.map((tool: any, index: number) => (
								<div
									key={index}
									className="border rounded-md p-4 bg-card hover:shadow-sm transition-shadow"
								>
						...
						<Tooltip>
							...
							<TooltipContent>
								<p className="text-xs max-w-xs">
									MCP configuration enables tools like shell commands, web
									scraping, and search functionalities.
								</p>
							</TooltipContent>
						</Tooltip>
					</TooltipProvider>
