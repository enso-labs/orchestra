...
const actionTypes = [
  { value: 'api', label: 'API Call', description: 'Make HTTP requests to external APIs' },
  { value: 'function', label: 'Function', description: 'Execute custom JavaScript functions' },
  { value: 'webhook', label: 'Webhook', description: 'Send data to webhook endpoints' },
  { value: 'custom', label: 'Custom', description: 'Define custom action logic' },
];
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {actions.map((action) => (
          <div key={action.id} className="relative group">
            <Badge
              variant={action.enabled ? getActionTypeColor(action.type) : 'outline'}
              className={`cursor-pointer transition-all hover:shadow-md ${
                selectedAction?.id === action.id ? 'ring-2 ring-ring ring-offset-1' : ''
        ...
        <div className="border rounded-lg p-4 bg-card">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <Badge variant={getActionTypeColor(selectedAction.type)} className="text-xs">
                {selectedAction.type}
              </Badge>
            </div>
            <div className="flex items-center gap-2">
            ...
            {selectedAction.type === 'webhook' && (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Input type="password" placeholder="Optional webhook secret" />
                </div>
              </div>
            )}
