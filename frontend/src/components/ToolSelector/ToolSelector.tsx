...
export function ToolSelector() {
	const { payload, useToolsEffect, useMCPEffect, useA2AEffect } =
		useChatContext();
	const {
		toolsByCategory,
		filteredTools,
	...
	return (
		<>
			...
			<Dialog open={isMenuOpen} onOpenChange={handleMenuOpen}>
				<DialogContent
					className={`${styles.toolModalContent} sm:max-w-[600px] md:max-w-[800px] h-auto max-h-[90vh] overflow-hidden p-0`}
				>
					{isAddingA2A ? (
						<A2AEditor />
					) : testingTool ? (
