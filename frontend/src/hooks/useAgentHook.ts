...
export default function useAgentHook() {
	...
	const handleUpdateAgent = async (id: string) => {
		try {
			...
			const response = await createSetting({
				name:
					agentDetails.name.replace(/\s+/g, "_").toLowerCase() +
					":" +
			...
			alert(
				`Agent created revision ${agentResponse.data.agent.name} successfully`,
			);
		} catch (error) {
	...
	const useEffectGetFilteredAgents = (
		searchTerm: string,
