...
export default function useToolHook() {
	...
	const { setTestFormValues, setTestingTool, setHasSavedA2A, setSwaggerSpec } =
		actions;
	...
	const toolsByCategory = filteredTools.reduce(
		(acc: Record<string, any[]>, tool: any) => {
			const category =
				tool.tags && tool.tags.length > 0 ? tool.tags[0] : "General";
	...
	const useSpecEffect = () => {
		useEffect(() => {
			...
			async function fetchSpec() {
				...
				const tool = await convertSpecToTool("Airtable", "Airtable", spec, {
