...
export default function DocumentManager() {
		...
		try {
			...
			if (createdCollection && createdCollection.uuid) {
				...
				const lastCollection =
					updatedCollections[updatedCollections.length - 1];
	...
	const handleDeleteCollection = async () => {
		...
		const selectedCollectionName =
			collections.find(
				(collection: Collection) => collection.uuid === selectedCollection,
			)?.name || "this collection";

		if (
			window.confirm(
				`Are you sure you want to delete "${selectedCollectionName}"? This action cannot be undone and will permanently remove all documents in this collection.`,
			)
		) {
			try {
				...
				if (collections.length > 0) {
					if (collectionId) {
						...
						if (foundCollection) {
							...
							navigate(`/collections/${collections[0].uuid}`, {
								replace: true,
							});
						}
					} else {
							...
							title={
								collections.find(
									(collection: Collection) =>
										collection.uuid === selectedCollection,
								)?.name ?? ""
							}
