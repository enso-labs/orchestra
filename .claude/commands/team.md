...
**Usage:** `/team query="<feature description>" subagents=<N>`
...
## Output
Write your complete proposal to: `PROPOSAL_<PERSONA_NAME>.md`
```
...
## Proposals to Review
You have received $ARGUMENTS.subagents independent proposals:
...
## Output
Write the complete council review to: `REVIEW.md`
```
...
## Output
Write the complete task contract to: `TASKS.md`
```
...
## Your Contract
`TASKS.md` is your binding agreement. Every checkbox is a commitment.

## Execution Rules
1. **Follow the order**: Tasks are sequenced for a reason
2. **One task at a time**: Complete fully before moving on
3. **Mark progress**: Update TASKS.md as you complete items
...
- Include error handling per REVIEW.md risk assessment
...
1. Final verification against REVIEW.md
...
- [ ] Edge cases from REVIEW.md handled
...
## Output
Append validation results to: `TASKS.md` under `## Validation Results`
```
...
|-------|-------|--------|------------|
| 0 | — | INITIAL_REPORT | Read plan.md |
| 1 | INITIAL_REPORT + query | PROPOSAL_*.md | Generate N agent proposals |
| 2 | All proposals | REVIEW.md | Council synthesis |
| 3 | REVIEW.md | TASKS.md | Create task contract |
| 4 | TASKS.md | Code changes | Implement features |
| 5 | Implementation | Validation | Verify & document |