...
export function useChatReducer() {
	...
	const actions = {
		...
		setToolCallMessage: (message: any) =>
			dispatch({
				type: ChatActionType.SET_TOOL_CALL_MESSAGE,
				payload: message,
			}),

		setIsToolCallInProgress: (inProgress: boolean) =>
			dispatch({
				type: ChatActionType.SET_IS_TOOL_CALL_IN_PROGRESS,
				payload: inProgress,
			}),

		setCurrentToolCall: (toolCall: any) =>
			dispatch({
				type: ChatActionType.SET_CURRENT_TOOL_CALL,
				payload: toolCall,
			}),

		setSelectedToolMessage: (message: any) =>
			dispatch({
				type: ChatActionType.SET_SELECTED_TOOL_MESSAGE,
				payload: message,
			}),
		...
		resetState: () => dispatch({ type: ChatActionType.RESET_STATE }),
	};
