...
async def test_api_token_lifecycle(async_client: AsyncClient):
    ...
    # 2. Create API Token
    response = await async_client.post("/api/tokens", json={"name": "Integration Test Token"}, headers=headers)
    ...
    # 4. Use API Token
    response = await async_client.get("/api/tokens", headers={"x-api-key": api_token_str})
