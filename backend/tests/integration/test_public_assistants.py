...
@pytest.mark.asyncio
async def test_create_assistant_with_files(async_client: AsyncClient):
    """Test creating assistant with files via API."""
    # Login
    login_data = {"email": "admin@example.com", "password": "test1234"}
    response = await async_client.post("/api/auth/login", json=login_data)
    if response.status_code != 200:
    ...
    try:
        ...
        assert assistants[0]["files"]["/config.json"] == '{"key": "value"}'
        assert assistants[0]["files"]["/script.py"] == "print('hello')"
    finally:
        await async_client.delete(f"/api/assistants/{assistant_id}", headers=headers)


@pytest.mark.asyncio
async def test_update_assistant_files(async_client: AsyncClient):
    """Test updating assistant's files via API."""
    # Login
    login_data = {"email": "admin@example.com", "password": "test1234"}
    response = await async_client.post("/api/auth/login", json=login_data)
    if response.status_code != 200:
...
@pytest.mark.asyncio
async def test_public_assistant_excludes_files(async_client: AsyncClient):
    """Test that public assistants do NOT expose files to non-owners."""
    # Login
    login_data = {"email": "admin@example.com", "password": "test1234"}
    response = await async_client.post("/api/auth/login", json=login_data)
    if response.status_code != 200:
    ...
    try:
        ...
        assert response.status_code == 200
        data = response.json()["assistant"]
        # CRITICAL: files is owner-only and must NOT be exposed publicly
        assert "files" not in data

        # But owner can still see files via authenticated search
        response = await async_client.post(
            "/api/assistants/search",
            json={"filter": {"id": assistant_id}},
            headers=headers,
