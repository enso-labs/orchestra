"""Tests for cron_llm_invoke TaskIQ dispatch logic."""
...
async def test_dispatches_to_taskiq_when_distributed(task_dict_with_thread):
    ...
    with patch("src.workers.tasks.run_agent_stream", mock_task):
        await cron_llm_invoke(task_dict=task_dict_with_thread, user_id="u1", title="test job")
...
async def test_in_process_when_not_distributed(task_dict_with_thread):
    ...
    with (
        ...
        await cron_llm_invoke(task_dict=task_dict_with_thread, user_id="u1", title="test")
...
async def test_generates_uuid_when_no_thread_id(task_dict_without_thread):
    ...
    with patch("src.workers.tasks.run_agent_stream", mock_task):
        await cron_llm_invoke(task_dict=task_dict_without_thread, user_id="u1", title="test")
