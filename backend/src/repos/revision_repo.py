...
class RevisionRepo:
    ...
    async def get_by_id(self, revision_id: str) -> Optional[Revision]:
        ...
        stmt = (
            select(Revision)
            .options(joinedload(Revision.setting))
    ...
    async def get_by_revision_number(
        self, agent_id: str, revision_number: int
    ) -> Optional[Revision]:
    ...
    async def get_latest_revision(self, agent_id: str) -> Optional[Revision]:
        ...
        stmt = (
            select(Revision)
            .where(Revision.agent_id == agent_id)
    ...
    async def create(
        self, agent_id: str, settings_id: str, name: str = None, description: str = None
    ) -> Revision:
        """Create a new revision for an agent."""
        try:
            # Verify agent belongs to the user
            stmt = select(Agent).where(
                Agent.id == agent_id, Agent.user_id == self.user_id
            )
            result = await self.db.execute(stmt)
            ...
            if not agent:
                raise ValueError(
                    f"Agent with ID {agent_id} not found or doesn't belong to user"
                )
    ...
    async def set_active_revision(
        ...
        # Verify agent belongs to the user
        stmt = select(Agent).where(Agent.id == agent_id, Agent.user_id == self.user_id)
        result = await self.db.execute(stmt)
        agent = result.scalars().first()
        ...
        if not revision:
            raise ValueError(
                f"Revision {revision_number} not found for agent {agent_id}"
            )
