...
def init_a2a_tools(
    ...
    for key, config in config.items():
        card = A2ACardResolver(base_url=config.base_url, agent_card_path=config.agent_card_path).get_agent_card()

        async def send_task(query: str, config=config):
            return await a2a_builder(base_url=config.base_url, query=query, thread_id=thread_id)
