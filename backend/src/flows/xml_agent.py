...
###########################################
def input_parser(
    messages: list[BaseMessage], llm_response_prefix: str = "\n\n ai_response:"
) -> str:
    xml_lines = ["<thread>"]
...
## Graph
###########################################
def create_enso_graph(
    tools: list[StructuredTool],
    mcp: dict = {},
    store: BaseStore = None,  # type: ignore
    checkpointer: BaseCheckpointSaver = None,  # type: ignore
):
    ...
    async def agent(state: AgentState, config: RunnableConfig):
        ...
        response = await model.ainvoke(
            [
                SystemMessage(
                    content=config.get("configurable", {}).get(
    ...
    workflow.compile(
        name="xml_agent", debug=True, checkpointer=checkpointer, store=store
    )
    return workflow
