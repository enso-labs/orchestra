...
class AgentController:
    ...
    async def query_thread(
        ...
        try:
            ...
            logger.info(
                f"Creating new thread with ID: {thread_id} and Query: {thread.query}"
            )
            config = {
                "thread_id": thread_id,
            ...
            await agent.abuilder(
                tools=[*thread.tools, thread.mcp, thread.a2a, thread.arcade],
    ...
    async def query_thread_agent(
        ...
        try:
            ...
            await agent.abuilder(
                tools=[
                    *settings.get("tools", []),
                    settings.get("mcp", None),
                    settings.get("a2a", None),
                    settings.get("arcade", None),
                ],
