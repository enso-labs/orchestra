...
class LLMWrapper:
    ...
    def choose_model(self, model_name: str):
        ...
        if 'openai' in model_name:
            openai_token = self.user_repo.get_token(key=UserTokenKey.OPENAI_API_KEY.name)
            if not openai_token:
            ...
            anthropic_token = self.user_repo.get_token(key=UserTokenKey.ANTHROPIC_API_KEY.name)
            ...
            groq_token = self.user_repo.get_token(key=UserTokenKey.GROQ_API_KEY.name)
            if not groq_token:
            ...
            gemini_token = self.user_repo.get_token(key=UserTokenKey.GEMINI_API_KEY.name)
            if not gemini_token:
