...
class TokenCreate(BaseModel):
    ...
    def validate_key(cls, v):
        if v not in UserTokenKey.values():
            raise ValueError(
                f"Invalid key. Must be one of: {', '.join(UserTokenKey.values())}"
            )
        return v
...
def create_token(
    ...
    try:
        ...
        # Create new token
        token = Token(user_id=user.id, key=token_data.key, value=encrypted_value)

        # Check if token already exists
        existing_token = (
            db.query(Token).filter_by(user_id=user.id, key=token_data.key).first()
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Error creating token"
        )
...
def delete_token(
    key: str, user: User = Depends(verify_credentials), db: Session = Depends(get_db)
):
    try:
        # Find and delete token
        result = db.query(Token).filter_by(user_id=user.id, key=key).delete()
...
async def get_tokens(
    user: User = Depends(verify_credentials), db: Session = Depends(get_db)
):
    try:
