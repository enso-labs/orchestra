...
async def list_agent_threads(
    ...
    try:
        ...
        agent = Agent(
            config={"user_id": user.id, "agent_id": agent_id}, user_repo=user_repo
        )
...
async def agent_existing_thread(
    ...
    try:
        controller = AgentController(db=db, user_id=user.id, agent_id=agent_id)
        return await controller.query_thread_agent(
            request=request, query=body.query, thread_id=thread_id
        )
    except Exception as e:
        return JSONResponse(
            content={"error": str(e)}, status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
