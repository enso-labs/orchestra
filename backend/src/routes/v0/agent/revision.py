...
def get_agent_revision(
    ...
    revision = revision_repo.get_by_revision_number(
        agent_id=agent_id, revision_number=revision_number
    )
    ...
    return JSONResponse(
        content={"revision": revision.to_dict()}, status_code=status.HTTP_200_OK
    )
...
@router.put(
    ...
    responses={
        ...
        status.HTTP_404_NOT_FOUND: {"description": "Agent or revision not found"},
        status.HTTP_400_BAD_REQUEST: {"description": "Bad request"},
    },
)
def set_active_revision(
    agent_id: str = Path(..., description="The ID of the agent"),
    revision_number: int = Path(
        ..., description="The revision number to set as active"
    ),
    user: ProtectedUser = Depends(verify_credentials),
    db: Session = Depends(get_db),
):
    ...
    try:
        agent = revision_repo.set_active_revision(
            agent_id=agent_id, revision_number=revision_number
        )
...
@router.delete(
    ...
    tags=[TAG],
    responses={
        status.HTTP_204_NO_CONTENT: {"description": "Revision deleted successfully."},
        status.HTTP_404_NOT_FOUND: {"description": "Revision not found"},
        status.HTTP_400_BAD_REQUEST: {"description": "Cannot delete active revision"},
    },
)
