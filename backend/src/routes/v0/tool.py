...
@router.get(
    ...
    responses={
        status.HTTP_200_OK: {
            ...
            "content": {"application/json": {"example": tools_response}},
        }
    },
)
def list_tools(
...
@router.get(
    ...
    responses={
        status.HTTP_200_OK: {
            ...
            "content": {"application/json": {"example": ARCADE_RESPONSE_EXAMPLE}},
        }
    },
)
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            res = await client.get(
                ...
                params={"toolkit": toolkit, "limit": limit, "offset": offset},
            )
            res.raise_for_status()
            arcade_tools = res.json()
        return JSONResponse(content=arcade_tools, status_code=status.HTTP_200_OK)
...
@router.get(
    ...
    responses={
        status.HTTP_200_OK: {
            "description": "All capabilities.",
            "content": {
                "application/json": {"example": ARCADE_RESPONSE_EXAMPLE["items"][0]}
            },
        }
    },
)
...
class MCPInfo(BaseModel):
    ...
    model_config = {"json_schema_extra": {"example": MCP_REQ_BODY_EXAMPLE}}
    ...
    try:
        ...
        for server_name, server in body.a2a.items():
            try:
                ...
                results.append(
                    {"error": str(server_error), "base_url": server.base_url}
                )

        return JSONResponse(
            content={"agent_cards": results}, status_code=status.HTTP_200_OK
        )
    except Exception as e:
        return JSONResponse(
            content={"error": str(e)}, status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
...
@router.post(
    ...
    responses={
        status.HTTP_200_OK: {
            ...
            "content": {
                "application/json": {
                    "example": {"result": "Tool execution result", "success": True}
                }
            },
        },
        ...
        status.HTTP_500_INTERNAL_SERVER_ERROR: {
            ...
            "content": {
                "application/json": {
                    "example": {"error": "Internal server error", "success": False}
                }
            },
        },
    },
...
async def invoke_tool(
    ...
    try:
        ...
        return JSONResponse(
            content={"output": output, "success": True}, status_code=status.HTTP_200_OK
        )
