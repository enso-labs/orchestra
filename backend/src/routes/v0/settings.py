...
async def list_settings(
    user: User = Depends(verify_credentials), db: AsyncSession = Depends(get_async_db)
):
    """List all settings."""
...
@router.post(
    "/settings",
    response_model=SingleSettingResponse,
...
async def update_setting(
    ...
    updated_setting = await repo.update(
        setting_id, setting.model_dump(exclude_unset=True)
    )
...
async def upsert_setting_by_slug(
    ...
    repo = SettingsRepo(db, user.id)
    return {
        "setting": await repo.upsert_by_slug(name=setting.name, value=setting.value)
    }
